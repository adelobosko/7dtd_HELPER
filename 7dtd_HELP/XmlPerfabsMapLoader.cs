...
{
    public class XmlPerfabsMapLoader : IMapLoader
    {
        ...
        public List<MapPoint> LoadMapPoints(string filename)
        {
            try
            {
                ...
                var perfabs = xmlDoc.Element(_root);
                if(perfabs == null)
                    throw new Exception($"element \"{_root}\" did not find.");


                var decorations = new List<MapPoint>();
                decorations.AddRange(perfabs.Elements(_element).Select(GetMapPoint).Where(mapPoint => mapPoint != null));
